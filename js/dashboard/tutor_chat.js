class Chat{
    ...
    constructor() {
        ...
        this._fetchContacts(true)


    }


    _fetchContacts(websocketPolling) {

        this._getContacts(Laravel.userId,(contacts) => {
    ...
    _renderContact(contact, container) {
        ...
        var itemClone = this.listItemTemplate.cloneNode(true).querySelector('a');

        itemClone.setAttribute('data-id', contact.id);

        // itemClone.setAttribute('href',tutorProfilePageUrl);

        // itemClone.querySelector('#contactName').setAttribute('href',tutorProfilePageUrl);

        // itemClone.querySelector('#contactProfileImg').setAttribute('href',tutorProfilePageUrl);

        if(contact.role === 'TUTOR'){
            itemClone.querySelector('#contactName').classList.add("tutor-name-color");
        }else if(contact.role === 'ADMIN'){
            itemClone.querySelector('#contactName').classList.add("admin-name-color");
        }else{
            itemClone.querySelector('#contactName').classList.add("student-name-color");
        }
    ...
    _setAsRead() {
        ...
        if (this.currentChatData.unread > 0) {
            ...
            for (const message of this.currentChatData.messages) {

                if (message.receiver_id === Laravel.userId)
    ...
    _renderChatMessage(chat, container) {
        ...
        console.log("INFO",this.currentChatData.profile_image);

        // console.log("INFO", Laravel);
        ...
        if (chat.receiver_id == Laravel.userId) {
            ...
            if (chat.type === 'text') {
                ...
                itemClone.querySelector('.time').innerHTML = new moment(chat.created_at).format('hh:mm A');
    ...
    _onAttachmentChange(event, type='image') {
        ...
        console.log('_onAttachmentChange', event)
        ...
        if (input.files && input.files[0]) {
            ...
            const attachment = {
                ...
                created_at: new moment().format('hh:mm'),
    ...
    _onContactListClick(event) {
        ...
        if (contactElement) {
            ...
            this.currentChatData = this._getDataById(contactId);
