...
class Chat {
  ...
  _renderContactTitle() {
    ...
    contactTitle.querySelector('.profile').setAttribute('src', `/storage/${this.currentChatData.profile_image}`);
  ...
  _renderChatMessage(chat, container) {
    ...
    if (chat.receiver_id == Laravel.userId) {
      ...
      if (chat.type === 'text') {
        ...
        itemClone.querySelector('.time').innerHTML = new moment(chat.created_at).format('hh:mm');
  ...
  _onAttachmentChange(event, type='image') {
    ...
    if (input.files && input.files[0]) {
        ...
        _submitAttachment(attachment, (message) => {
            console.log(message)
            this._renderChatMessage(message, this.chatContentContainer.querySelector('.os-content'));
            this._updateChatScroll();
